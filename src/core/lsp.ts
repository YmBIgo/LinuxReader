...
export async function getFunctionContentFromLineAndCharacter(filePath: string, line: number, character: number) {
...
export async function getFileLineAndCharacterFromFunctionName(
    ...
    const simplfiedFunctionName = functionName.split(",")[0].replace(/^[\s\t]*/g, "");
    ...
    for (let i in fileContentArray) {
        ...
        const functionIndex = row.indexOf(simplfiedFunctionName);
        if (functionIndex >= 0){
